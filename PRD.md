...
- 不接入真实蓝牙玩偶或 IoT 设备。
- 不接入真实 GPS 或轨迹采集。
- 不在**客户端本地**跑图像生成模型；若产品需要「打卡后的装饰小物」，通过**服务端**可选接口生成，且仅作为**日常灵感件（低门槛）**定位，与联名/在地典藏（高上限）在数据字段与 UI 上分轨（见 3.2 素材分层）。用户生活照片上传仍为重要来源之一。
...
为对齐「任天堂式」体验——**人人能玩（低门槛）**与**深度投入有回报（高上限）**——橱柜内物件按来源与稀缺规则分层展示与打点，而非仅按单张图观感定价值。

- **日常灵感件（AI 生成）**：打卡/日记链路可触发的服务端生成装饰图；`assetSource = ai_daily`，`scarcity = soft_capped`（可配置日上限或冷却）；UI 使用轻量获得反馈，收纳在「日常灵感」区，**不与**典藏共用稀有度主叙事。
- **记忆件（用户上传）**：用户相册上传的配件；`assetSource = user_upload`；强调真实生活与情感价值。
- **典藏件（联名 / 在地 / 活动）**：`assetSource ∈ { collab, geo_limited, campaign }`，`scarcity = limited`；来自品牌联名素材、线下 NFC、地理围栏或活动码等；UI 使用典藏陈列区与更强的收集仪式感（震动、动效分级可与日常件区分）。
